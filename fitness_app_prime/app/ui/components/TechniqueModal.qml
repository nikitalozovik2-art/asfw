...
Item {
  ...
  // 0..1 open progress (drives fade/slide)
  property real openP: 0.0
  ...
  NumberAnimation {
    ...
    onStopped: {
      if (modal.openP <= 0.001) modal.visible = false
    }
  }

  // Strong dim + haze (pseudo blur), animated
  Rectangle { anchors.fill: parent; color: Qt.rgba(0, 0, 0, 0.58 * modal.openP) }
  Rectangle { anchors.fill: parent; color: Qt.rgba(1, 1, 1, 0.10 * modal.openP) }

  // click outside closes
  MouseArea {
    anchors.fill: parent
    enabled: modal.openP > 0.0
  ...
  Rectangle {
    ...
    x: (parent.width - width - 28) + (1.0 - modal.openP) * 40   // slide-in
    y: (parent.height - height) / 2
    radius: 26
    color: "#ffffff"
    opacity: modal.openP
    scale: 0.985 + 0.015 * modal.openP
    antialiasing: true
    border.width: 1
    border.color: Qt.rgba(0, 0, 0, 0.08)

    // absorb clicks so outside area doesn't close
